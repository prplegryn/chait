...
class AppLogger {
  ...
  Future<void> start() async {
    ...
    try {
      ...
      _privateDirectory = dir.path;
      _privateFile = File('${dir.path}/$_fileName');
      _privateSink = _privateFile!.openWrite(mode: FileMode.writeOnlyAppend);
      await _deleteOldPrivateLogs(dir);
      for (final entry in _earlyEntries) {
        _privateSink?.write(entry);
      }
      _earlyEntries.clear();
      info('logger', 'started private=$privatePath publicHint=$bestVisiblePath');
    } catch (error, stack) {
      _earlyEntries.add(_format('ERROR', 'logger', 'start failed: $error', stack));
    }
  }
  ...
  Future<void> _deleteOldPrivateLogs(Directory dir) async {
    try {
      final logs = dir
          .listSync()
          .whereType<File>()
